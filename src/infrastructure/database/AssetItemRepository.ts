...
export class AssetItemRepository implements IAssetItemRepository {
    ...
    async checkQtyAvailable(
        assetItemCode: string,
        quantity: number
    ): Promise<boolean> {
        const assetItem = await db
            .selectFrom('asset_items')
            .leftJoin('asset_requests', (join) =>
                join
                    .onRef('asset_items.asset_code', '=', 'asset_requests.asset_item_code')
                    .on('asset_requests.status', 'in', ['PENDING', 'APPROVED'])
            )
            .select([
                'asset_items.asset_code',
                'asset_items.quantity as assetItem_quantity',
                sql<number>`COALESCE(SUM(asset_requests.quantity), 0)`.as('assetRequest_qty'),
                sql<number>`asset_items.quantity - COALESCE(SUM(asset_requests.quantity), 0)`.as('remaining_quantity'),
            ])
            .where('asset_items.asset_code', '=', assetItemCode)
            .groupBy([
                'asset_items.asset_code',
                'asset_items.quantity',
            ])
            .executeTakeFirst();
